...
if ($connection) {
	mysqli_query($connection, "UPDATE `portfolio` SET `name` = '$name'  WHERE `portfolio`.`id` = '$id';");

	if ($photo) {
		mysqli_query($connection, "UPDATE `portfolio` SET `photo` = '$photo' WHERE `portfolio`.`id` = '$id';");
	}
}
