...
if($connection) {
   mysqli_query($connection, "UPDATE `video-instruction` SET `name` = '$name', `description` = '$description'  WHERE `video-instruction`.`id` = '$id';"); 

   if($photo) {
      mysqli_query($connection, "UPDATE `video-instruction` SET `photo` = '$photo' WHERE `video-instruction`.`id` = $id';"); 
   }
}
...
// echo '<script>location.replace("/admin/pages/main/#video-instruction");</script>';
