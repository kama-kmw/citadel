...
if ($connection) {
	mysqli_query($connection, "UPDATE `team` SET `name` = '$name', `status` = '$status'  WHERE `team`.`id` = '$id';");

	if ($photo) {
		mysqli_query($connection, "UPDATE `team` SET `photo` = '$photo' WHERE `team`.`id` = '$id';");
	}
}
...
echo '<script>location.replace("/admin/pages/main/#team");</script>';
